...
def test_crisis_event_has_updates():
    ...
    if not crisis_type:
        print("❌ FAIL: No crisis detected with low food")
        return False

    print(f"✓ Crisis detected: {crisis_type}")

    # Generate crisis event
    try:
        event_data = generate_crisis_event(game_state, crisis_type)
    except Exception as e:
        print(f"❌ FAIL: Error generating crisis event: {e}")
        return False

    # Check if updates block exists
    if 'updates' not in event_data:
        print(f"❌ FAIL: Crisis event does not contain 'updates' block")
        print(f"Event data keys: {list(event_data.keys())}")
        return False

    print(f"✓ Crisis event contains 'updates' block")

    # Check if updates block is non-empty
    updates = event_data.get('updates', {})
    if not updates:
        print(f"❌ FAIL: Updates block is empty")
        return False

    print(f"✓ Updates block contains {len(updates)} entries:")
...
def test_crisis_updates_validation():
    ...
    crisis_type = detect_crisis(game_state)
    if not crisis_type:
        print("❌ FAIL: No crisis detected with low wealth")
        return False
    ...
    updates = event_data.get('updates', {})
    if not updates:
        print(f"⚠️  SKIP: Updates block is empty (AI may not have generated updates)")
    ...
    if not cleaned_updates:
        print(f"❌ FAIL: No valid updates after validation")
        return False

    print(f"✓ Validation passed with {len(cleaned_updates)} valid updates")
...
def test_crisis_updates_application():
    ...
    if crisis_type != 'famine':
        print(f"⚠️  SKIP: Expected famine crisis, got {crisis_type}")
        return True

    print(f"✓ Famine crisis detected")
    ...
    try:
        ...
        if not updates:
            print(f"⚠️  SKIP: No updates generated by AI")
            return True
        ...
        if cleaned_updates:
            ...
            print(f"⚠️  SKIP: No valid updates to apply")
            return True

    except Exception as e:
        print(f"❌ FAIL: Error applying updates: {e}")
        return False
    ...
    print(f"✓ Population: {initial_population} → {final_population} ({population_change:+d})")
    print(f"✓ Food: {initial_food} → {final_food} ({food_change:+d})")

    # Expect negative changes for famine
    if population_change >= 0 and food_change >= 0:
        print(f"⚠️  WARNING: Expected negative changes for famine, but no losses occurred")
...
def test_multiple_crisis_types():
    ...
    for crisis_type, conditions in crisis_scenarios:
        ...
        try:
            ...
            if updates:
                print(f"✓ {crisis_type}: Generated {len(updates)} updates")
                results.append(True)
            else:
                print(f"⚠️  {crisis_type}: No updates generated")
